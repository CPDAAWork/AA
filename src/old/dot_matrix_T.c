...
int main(int argc, char *argv[]){
  ...
  for(int i = 0; i<iter; i++){
    ...
    timer1 = dotMulti_T(A,B,C,size,size);

    //printf("%lld", (timer1));
    print_papi();
  }

  printf("\nTYPE Transpose :: j-k-i\n");
  if(papi_init() == 0)
    return 0;
  ...
  for(int i = 0; i<iter; i++){
    ...
    timer1 = dotMulti3_T(A,B,C,size,size);

    //printf("%lld", (timer1));
    print_papi();
  }

  printf("\nTYPE Transpose :: k-i-j\n");
  if(papi_init() == 0)
    return 0;
  ...
  for(int i = 0; i<iter; i++){
    ...
    timer1 = dotMulti2_T(A,B,C,size,size);
